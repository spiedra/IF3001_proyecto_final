CREATE OR REPLACE FUNCTION ESTUDIANTE.fn_INSERTAR_ESTUDIANTE
(
...
BEGIN
 ...
 ELSE
		...
		SET local_id_direccion= LASTVAL;
...
CREATE OR REPLACE FUNCTION ADMINISTRACION.fn_INSERTAR_BECA
(param_TIPO_BECA INT) RETURNS INT
LANGUAGE plpgsql
AS $$
BEGIN

	IF EXISTS (SELECT ID_BECA FROM ADMINISTRACION.tb_BECA WHERE TIPO_BECA=param_TIPO_BECA) THEN
...
ALTER PROCEDURE ADMINISTRACION.fn_INSERTAR_CARRERA
...
ALTER PROCEDURE ADMINISTRACION.fn_INSERTAR_CURSO
...
ALTER PROCEDURE ADMINISTRACION.fn_INSERTAR_SEDE
...
CREATE OR REPLACE FUNCTION ESTUDIANTE.fn_INSERTAR_SEDE_ESTUDIANTE
(param_ID_ESTUDIANTE INT,
param_NOMBRE_SEDE VARCHAR(32)) RETURNS INT
LANGUAGE plpgsql
AS $$
...
--------------------------------------------------------
CREATE OR REPLACE FUNCTION ESTUDIANTE.fn_INSERTAR_TELEFONO_ESTUDIANTE
(
...
AS $$
DECLARE 
local_id_telefono_verificar INT;
local_ID_TELEFONO INT;
BEGIN
 ...
 IF EXISTS (SELECT ID_ESTUDIANTE, ID_TELEFONO FROM ESTUDIANTE.tb_ESTUDIANTE_TELEFONO WHERE ID_TELEFONO=local_id_telefono_verificar AND ID_ESTUDIANTE=param_ID_ESTUDIANTE) THEN
...
CREATE OR REPLACE FUNCTION ESTUDIANTE.fn_INSERTAR_CURSO_ESTUDIANTE
(
param_ID_ESTUDIANTE INT,
param_NOMBRE_CURSO INT
) RETURNS INT
LANGUAGE plpgsql
AS $$
...
SET local_ID_CURSO='SELECT ID_CURSO FROM ADMINISTRACION.tb_CURSO WHERE NOMBRE_CURSO=@param_NOMBRE_CURSO';
  ...
  IF EXISTS (SELECT ID_ESTUDIANTE, ID_CURSO FROM ESTUDIANTE.tb_ESTUDIANTE_CURSO WHERE ID_ESTUDIANTE=param_ID_ESTUDIANTE AND ID_CURSO=local_ID_CURSO) THEN
...
CREATE OR REPLACE FUNCTION ESTUDIANTE.fn_INSERTAR_CARRERA_ESTUDIANTE
(
param_ID_ESTUDIANTE INT,
param_NOMBRE_CARRERA VARCHAR(32)
) RETURNS INT
...
IF EXISTS (SELECT ID_ESTUDIANTE, ID_CARRERA FROM ESTUDIANTE.tb_ESTUDIANTE_CARRERA WHERE ID_ESTUDIANTE=@param_ID_ESTUDIANTE AND ID_CARRERA=@local_ID_CARRERA) THEN
